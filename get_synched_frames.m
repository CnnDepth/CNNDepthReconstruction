...
function frameList = get_synched_frames(sceneDir)
  ...
  files_struct = dir(sceneDir);
  files_array = [];
  for i = 1 : length(files_struct)
    files_array{i} = [];
    files_array(i) = files_struct(i).name;
  end
  files = files_array;
  files(end) = [];
...
sceneDir = argv(){1};
synched_frames = get_synched_frames(sceneDir);
file = fopen(strcat(sceneDir, "/match.txt"), "w");
for i = 1 : length(synched_frames)
  fprintf(file, "rgb: %s depth: %s accel: %s\n", synched_frames(i).rawRgbFilename, synched_frames(i).rawDepthFilename, synched_frames(i).accelFilename);
end
