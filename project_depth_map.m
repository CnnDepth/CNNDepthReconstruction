...
function [depthOut, rgbUndistorted] = project_depth_map(imgDepth, rgb)
  ...
  depthOut = uint16(depthOut * 65535 / maxDepth);
end


% Usage: project_depth_map.m depthSource rgbSource depthDestination rgbDestination
% depthSource is a path to source depth map
% rgbSource is a path to corresponding RGB image
% depthDestination is a path to save projected depth map
% rgbDestination is a path to save processed RGB image

depthSource = argv(){1};
rgbSource = argv(){2};
depthDestination = argv(){3};
rgbDestination = argv(){4};
depthImg = imread(depthSource);
rgbImg = imread(rgbSource);
[depthOut, rgbUndistorted] = project_depth_map(depthImg, rgbImg);
depthOut = depthOut(45:471, 41:601);
rgbUndistorted = rgbUndistorted(45:471, 41:601, :);
imwrite(depthOut, depthDestination);
imwrite(rgbUndistorted, rgbDestination);
